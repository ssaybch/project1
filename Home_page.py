...
def chembl_func(smiles):
    ...
    result_df2 = pd.DataFrame(data = result_9606_GENE_chemblid_proba, columns=['GENE','UNIPROT_ID','probability']) #임시로 dataframe 필터링
...
input_string  = st.text_input("","C1=CC(=CC=C1CO)O", help="올바르지 않은 SMILES일 경우 에러가 출력됩니다.")
...
st.write("TPSA: ", round(tpsa, 3), "(단, S와 P의 영향은 고려하지 않았으며 SwissADME의 결과와 동일함.)")
